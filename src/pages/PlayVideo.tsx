...
const RecentPostCard = ({ video, onClick }: { video: any, onClick: (videoId: string) => void }) => (
    <div onClick={() => onClick(video.id)} className="group w-64 flex-shrink-0 cursor-pointer">
      <div className="relative w-full aspect-video rounded-lg overflow-hidden bg-[#232B2B] border border-[#4A5568] group-hover:border-[#64748B] transition-all">
        <video className="w-full h-full object-cover" preload="metadata" muted>
          <source src={video.Url} type="video/mp4" />
        </video>
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
          <FaPlay className="text-gray-200 text-4xl" />
        </div>
        <div className="absolute top-2 left-2 bg-[#B22222] text-white text-xs font-bold px-2 py-1 rounded-md">
          New
        </div>
      </div>
      <div className="mt-2">
        <h3 className="text-gray-200 font-medium text-sm line-clamp-2">{video.Judul}</h3>
      </div>
    </div>
);
...
export function PlayVideo() {
  ...
  const randomUrls = [
    'https://viiukuhe.com/dc/?blockID=425121'
  ];
  ...
  if (id && !videoFound) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-gray-400 text-center p-4">
        <FaExclamationTriangle size={64} className="mb-4 text-[#B22222]" />
        ...
        <Link to="/" className="mt-6 inline-block bg-[#4A5568] hover:bg-[#64748B] text-white font-bold py-2 px-4 rounded transition-colors">
            Go to Homepage
        </Link>
      </div>
    );
  ...
  const PlayerView = () => (
    <div className="bg-[#232B2B] p-4 rounded-lg shadow-lg mb-8">
      <h1 className="text-2xl font-bold mb-4 text-center break-words text-[#9FB6D8]">{videoTitle}</h1>
      <div className="w-full aspect-video rounded-lg overflow-hidden shadow-lg border border-[#4A5568] flex items-center justify-center bg-black">
        {isBuffering && (
            <div className='text-center text-gray-300'>
                <FaSpinner className="animate-spin text-4xl text-[#64748B] mx-auto" />
      ...
      <div className="flex mt-4 mb-4 border border-[#4A5568] rounded-lg overflow-hidden">
        <input type="text" value={`https://${window.location.hostname}/play/${id}`} readOnly className="flex-1 p-3 bg-[#2C3539] text-gray-200 outline-none" />
        <button onClick={handleCopy} className="bg-[#4A5568] hover:bg-[#64748B] transition-colors text-white p-3">
          <FaCopy />
        </button>
      </div>
      <button onClick={handleDownloadClick} className="w-full bg-[#3D4852] hover:bg-[#4A5568] transition-colors text-white py-3 rounded-lg flex items-center justify-center font-semibold shadow-md">
  ...
  return (
    <div className="container mx-auto max-w-6xl p-4 sm:p-6 text-gray-200">
        ...
        <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {currentVideos.map((video) => (
                  <div onClick={() => handleCardClick(video.id)} key={video.id} className="group transition-all cursor-pointer">
                      <div className="relative w-full aspect-video rounded-lg overflow-hidden bg-[#232B2B] border border-[#4A5568] group-hover:border-[#64748B]">
                        <video className="w-full h-full object-cover" preload="metadata" muted>
                            <source src={video.Url} type="video/mp4" />
                        </video>
                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <div className="mt-2">
                        <h3 className="text-gray-200 font-medium text-sm sm:text-base line-clamp-2">{video.Judul}</h3>
                      </div>
                  </div>
              ))}
            </div>
          ...
          {totalPages > 1 && (
              <div className="flex items-center justify-between mt-8">
                <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="bg-[#3D4852] hover:bg-[#4A5568] transition-colors text-white py-2 px-4 rounded disabled:opacity-50 disabled:cursor-not-allowed">
                  Previous
                </button>
                <span className="text-gray-400">
                  Page {currentPage} of {totalPages}
                </span>
                <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="bg-[#3D4852] hover:bg-[#4A5568] transition-colors text-white py-2 px-4 rounded disabled:opacity-50 disabled:cursor-not-allowed">
                  Next
                </button>
              </div>
          )}
